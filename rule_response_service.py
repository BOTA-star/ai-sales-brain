...
def _evaluate_math_node(node: ast.AST) -> int | float:
    ...
    if (
        ...
        and type(node.op) in _ALLOWED_BINARY_OPERATORS
    ):
    ...
    if (
        ...
        and type(node.op) in _ALLOWED_UNARY_OPERATORS
    ):
        operand_value = _evaluate_math_node(node.operand)
        operation = _ALLOWED_UNARY_OPERATORS[type(node.op)]

        return operation(operand_value)

    raise ValueError("Biểu thức toán học không được hỗ trợ.")

def answer_simple_math(user_input: str) -> str:
    """
    Trả lời phép tính đơn giản bằng AST.
    """
    expression_text = extract_math_expression(user_input)
...
def extract_customer_name(user_input: str) -> str:
    """
...
def answer_customer_information(
    ...
    customer_name = extract_customer_name(clean_input)
    ...
    if any(
        ...
        return (
            opening
            + "Mình đã ghi nhận bạn đang cần hệ thống hỗ trợ đội kinh doanh "
            "xử lý lead và khách tiềm năng hiệu quả hơn. "
            "Bạn muốn ưu tiên phần nào trước: phân loại lead, gợi ý phản hồi cho sale, "
            "theo dõi trạng thái khách hàng hay tự động nhắc follow-up?"
        )
    ...
    if (
        ...
        return (
            opening
            + "Mình đã ghi nhận bạn cần một chatbot hỗ trợ hoạt động sale. "
            "Chatbot sẽ hỗ trợ tư vấn, thu thập thông tin và phân loại khách hàng, "
            "sau đó nhân viên sale sẽ kiểm tra và quyết định phản hồi cuối cùng. "
    ...
    if (
        ...
        if any(
            ...
            return (
                opening
                + "Mình đã ghi nhận bạn đang làm việc trong lĩnh vực AI và cần một chatbot "
                "để tăng năng suất công việc. "
                "Bạn muốn chatbot ưu tiên hỗ trợ phần nào: tra cứu thông tin, "
                "xử lý công việc lặp lại, hỗ trợ kỹ thuật hay quản lý công việc?"
            )

        return (
            opening
            + "Mình đã ghi nhận bạn cần một chatbot để tăng năng suất công việc. "
    ...
    if (
        ...
        return (
            ...
            "Bạn muốn chatbot ưu tiên tư vấn khóa học, giải đáp thắc mắc, "
            "nhắc lịch hay chăm sóc sau đăng ký?"
        )
    ...
    if (
        ...
        return (
            opening
            + "Bạn đang muốn sử dụng chatbot để hỗ trợ chăm sóc khách hàng. "
            "Vấn đề cần ưu tiên là tốc độ phản hồi, xử lý câu hỏi lặp lại, "
            "theo dõi khách hàng hay phân loại khách hàng?"
        )
    ...
    if (
        ...
        return (
            opening
            + "Mình đã ghi nhận bạn là thực tập sinh AI và muốn dùng chatbot để hỗ trợ công việc. "
    ...
    if "chatbot" in combined_context:
        return (
            ...
            "Bạn muốn chatbot hỗ trợ chính cho công việc nội bộ, bán hàng, "
            "marketing hay chăm sóc khách hàng?"
        )
...
def answer_by_rule(
    ...
    if intent == "bot_capability":
        return (
            "Hiện tại mình được thiết kế theo chế độ Strict RAG, nên chủ yếu trả lời "
            "dựa trên các tài liệu đã được nạp vào hệ thống. "
            "Nếu tài liệu không có thông tin phù hợp, mình sẽ thông báo là chưa tìm thấy "
            "thay vì tự dùng kiến thức bên ngoài để trả lời."
        )
...
def build_memory_candidate(
    ...
    if not matched_categories:
        logger.info(
            "Skip memory because no memory signal matched. input=%s",
            clean_input,
        )
        return None
...
def _get_memory_timestamp(
    ...
    AgentMemory hiện có thể trả createdAt thay vì timestamp.
    """
    timestamp_fields = (
        "updatedAt",
        "createdAt",
...
def _detect_recall_targets(
    ...
    """
    Xác định trường thông tin người dùng đang yêu cầu nhớ lại.
    """
...
def _filter_memories_by_target(
    ...
    matched_memories = [
        ...
        if _get_memory_concepts(memory) & expected_concepts
    ]
...
def _is_invalid_recall_value(
    ...
    return normalized.endswith(" gì") or normalized.endswith(" nào")
...
def answer_memory_recall(
    ...
    try:
        ...
        logger.exception(
            "Failed to recall customer memory. customer_id=%s",
            clean_customer_id,
        )
    ...
    if "identity" in targets:
        identity_memories = _filter_memories_by_target(
            ranked_memories,
            "identity",
        )
        ...
        for memory in identity_memories:
            ...
            customer_name = extract_customer_name(content)
    ...
    if "business" in targets:
        business_memories = _filter_memories_by_target(
            ranked_memories,
            "business",
        )
        ...
        for memory in business_memories:
            content = _clean_memory_content(memory)
            business_field = _extract_business_field(content)
    ...
    if "need" in targets:
        need_memories = _filter_memories_by_target(
            ranked_memories,
            "need",
        )
        ...
        for memory in need_memories:
            content = _clean_memory_content(memory)
            customer_need = _extract_need(content)
    ...
    if "priority" in targets:
        priority_memories = _filter_memories_by_target(
            ranked_memories,
            "priority",
        )
        ...
        for memory in priority_memories:
            content = _clean_memory_content(memory)
            current_priority = _extract_priority(content)
    ...
    if "problem" in targets:
        problem_memories = _filter_memories_by_target(
            ranked_memories,
            "problem",
        )

        if problem_memories:
            problem_content = _clean_memory_content(
                problem_memories[0]
            )
    ...
    if not fallback_contents:
        return "Mình tìm thấy memory nhưng chưa trích xuất được đúng thông tin bạn đang hỏi."
